   ...
   public static class Data
    {
        ...
        public static DataTable DS_SINHVIEN()
        {
            SqlConnection con = new SqlConnection(DB_connect.strcon);
            con.Open();
            SqlDataAdapter adap = new SqlDataAdapter("select row_number() over (order by MaSV) as STT, MaSV as [Mã Sinh Viên],TenSV as[Tên Sinh Viên],NgaySinh as[Ngày Sinh],MaLop as [Mã Lớp], MaKhoa as [Mã Khoa]  from SINHVIEN", con);
            DataTable ds = new DataTable();
            adap.Fill(ds);
            return ds;
        }
